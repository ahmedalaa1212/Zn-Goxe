...
window.loadUserData = async function () {
    try {
        ...
        if (d?.success) {
            ...
            // Mark the shared state as server-hydrated. Wallet and other
            // modules can use it immediately without issuing duplicate reads.
            window.__userStateHydrated = true;
            window.userState.last_sync_time = Date.now();
            window.dispatchEvent(
                new CustomEvent("userStateUpdated", {
                    detail: window.userState,
