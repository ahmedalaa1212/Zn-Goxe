...
(function initFarm() {
    ...
    // دالة تنسيق أرقام عملة ZN
    // 4 أرقام للأعداد أقل من 10، ورقمين للأعداد 10 فما فوق
    ...
    function setStoredBalance(newBalance, newUsdBalance) {
        ...
        if (newBalance !== undefined && newBalance !== null) {
            ...
            if (balEl) {
                balEl.innerText = `${formatZnBalance(val)} ZN`;
            }
        }
        ...
        if (newUsdBalance !== undefined && newUsdBalance !== null) {
            ...
            if (usdEl) {
                usdEl.innerText = formatUsdBalance(usdVal);
            }
        }
    ...
    window.fetchPlayerDataFromServer = async function(force = false) {
        ...
        try {
            let resData = await window.fetchAPI(
            ...
            if (resData && resData.success) {
                ...
                if (resData.cooldown_seconds) {
                    MIN_CLAIM_INTERVAL = resData.cooldown_seconds;
                }

                if (resData.game_config) {
                    if (
                        resData.game_config.daily_rewards &&
                        Array.isArray(resData.game_config.daily_rewards)
                    ) {
                        GAME_CONFIG.dailyRewards =
                            resData.game_config.daily_rewards;
                    }

                    if (resData.game_config.upgrade_costs) {
                        GAME_CONFIG.upgradeCosts =
                            resData.game_config.upgrade_costs;
                    }

                    if (resData.game_config.storage_config) {
                        GAME_CONFIG.storageConfig =
                            resData.game_config.storage_config;
                    }

                    if (resData.game_config.max_upgrades_per_level) {
                        GAME_CONFIG.maxUpgradesPerLevel =
                            resData.game_config.max_upgrades_per_level;
                    }

                    if (resData.game_config.daily_boost_reward) {
                        GAME_CONFIG.dailyBoostReward =
                            resData.game_config.daily_boost_reward;
                    }

                    if (resData.game_config.max_daily_boost_rate) {
                        GAME_CONFIG.maxDailyBoostRate =
                            resData.game_config.max_daily_boost_rate;
                    }

                    if (resData.game_config.boost_max_reward_coins) {
                        GAME_CONFIG.boostMaxRewardCoins =
                            resData.game_config.boost_max_reward_coins;
                    }
                }
    ...
    window.updateFarmUI = function() {
        ...
        if (rateEl) {
            let formattedRate =
                (hRate % 1 === 0)
                    ? hRate.toString()
                    : Number(hRate.toFixed(2)).toString();

            rateEl.innerHTML =
                `<span dir="ltr">${formattedRate} /h</span> ⚡`;
        }

        const stgLvl = parseInt(
        ...
        if (upgradeStgBtn) {
            ...
            if (stgLvl >= 9 || !nextCfg) {
                ...
                upgradeStgBtn.innerText =
                    `ترقية المخزن Lvl ${nextLvl} (${costStrZn} ZN${costStrUsd}) 📦`;

                upgradeStgBtn.disabled =
                    !canAfford ||
                    isUpgradingStorage;
        ...
        if (boostBtn) {
            ...
            if (lastBoost === todayStr) {
                ...
                boostBtn.innerHTML =
                    `<span style="font-size: 12px;">⏳</span><span style="font-size: 8px;">${getTimeUntilUTCMidnight()}</span>`;
            } else {
                if (!isBoosting) {
                    boostBtn.className =
                        "boost-btn";
                    ...
                    const boostText =
                        (
                            currentDailyBoostRate >=
                            GAME_CONFIG.maxDailyBoostRate
                        )
                            ? `+${GAME_CONFIG.boostMaxRewardCoins} ZN`
                            : `+${GAME_CONFIG.dailyBoostReward}/h`;

                    boostBtn.innerHTML =
                        `<span id="boost-icon">🚀</span><span id="boost-text">${boostText}</span>`;
                }
            }
        }
    ...
    window.farmIntervalId = setInterval(() => {
        ...
        if (
            ...
            storageTextEl.innerText =
                `${formatZnBalance(unclaim)} / ${maxC.toLocaleString('en-US', {maximumFractionDigits: 2})}`;
        }
        ...
        if (boostBtn) {
            ...
            if (lastBoost === todayStr) {
                ...
                boostBtn.innerHTML =
                    `<span style="font-size: 12px;">⏳</span><span style="font-size: 8px;">${timeLeftStr}</span>`;
            } else {
                if (!isBoosting) {
                    boostBtn.className =
                        "boost-btn";
        ...
        try {
            ...
            if (resData && resData.success) {
                ...
                if (resData.type === 'speed') {
                    ...
                    showToast(
                        `💰 تهانينا! تمت إضافة ${resData.reward_coins || GAME_CONFIG.boostMaxRewardCoins} عملة ZN إلى رصيدك مباشرة!`
                    );
                }
    ...
    window.handleMainClaim = async function() {
        ...
        try {
            ...
            if (resData && resData.success) {
                ...
                showToast(
                    `💰 تم تجميع ${resData.claimed_amount} عملة بنجاح!`
                );
